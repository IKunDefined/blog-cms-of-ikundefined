<template>
  <div>
    ...
    <el-table :data="categoryList" :stripe="true" >
      <el-tabal-column align="center" prop="_id" label="id"></el-tabal-column>
      <el-tabal-column align="center" prop="name" label="分类名称"></el-tabal-column>
      <el-tabal-column align="center" label="操作">
        <template slot-scope="scope">
          <el-button type="danger" @click.native="deleteCategory(scope.row._id)">删除</el-button>
        </template>
      </el-tabal-column>
    </el-table>
  </div>
...
export default {
  ...
  methods: {
    addCategory () {
      MessageBox.prompt('请输入需要添加的分类').then(({ value }) => {
        let body = {
          name: value
        }
        Axios.post('http://localhost:3000/admin/category/add', { body }).then(res => {
          if (res.data.code === 0) {
    ...
    deleteCategory (id) {
      MessageBox.confirm('是否要删除该分类', '确认删除', {
        ...
        Axios.post('http://localhost:3000/admin/category/delete', { body }).then(res => {
          if (res.data.code === 0) {
            MessageBox.alert(res.data.message)
